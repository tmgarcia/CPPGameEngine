...
{ 
	...
	shadowUV.x = (shadowUV.x+1)/2;
	shadowUV.y = (shadowUV.y+1)/2;
	shadowUV.z = (shadowUV.z+1)/2;
	float shadowTexel = texture(shadowMap, shadowUV.xy);
	if(abs(shadowTexel-shadowUV.z) > 0.00001f)
	{
		theFinalColor= (finalAmbient)* objectColor;
	}
	else
	{
		theFinalColor= (finalAmbient + vec4(brightness, brightness, brightness, 1))* objectColor + specularLighting;
	}
	//theFinalColor = vec4(shadowUV.x, shadowUV.x, shadowUV.x, 1);
	if(hasDiffuseMap>0)
	{
